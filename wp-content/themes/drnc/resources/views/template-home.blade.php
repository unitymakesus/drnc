      ...
      <div class="row">
        ...
        <section class="col m6 s12 section-news" role="region" aria-labelledby="social-title">
          ...
          <div class="events-list">
            ...
            {!! do_shortcode('[my_calendar_upcoming category="1,2,3" template=\'<time>{date format=\"M\"} <span>{date format=\"j\"}</span> {date format=\"Y\"}</time> <h3>{linking_title}</h3><p class=\"strong\">{location after="<br />"}{timerange}</p>\' before="0" after="2" show_today="yes" type="event" order="asc"]') !!}
          </div>
        </section>
      </div>
    </div>
